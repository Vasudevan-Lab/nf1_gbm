...
CONTEXT <- c("MES vs nonMES")
INPUT_DIRS <- c(sprintf("/NF1_patient/differential_expression/gsea-grouped/csv"))
OUTPUT_DIR <- sprintf("/NF1_patient/differential_expression/gsea-grouped")
...
ht = Heatmap(combined.df,
        name = "Enrichment Scores across celltypes",
...
combined.df <- bind_rows(fgsea.output.list, .id = "Clusters")
...
nes.df <- nes.df %>%
  group_by(pathway, Clusters) %>%
  pivot_wider(names_from = Clusters, values_from = NES) %>%
  group_by(pathway)
...
df.ordered <- combined.df %>%
  arrange(match(pathway, rownames(nes.df)[row_order]), match(Clusters, colnames(nes.df)))

df.ordered$pathway <- factor(df.ordered$pathway, levels = rownames(nes.df)[row_order])
df.ordered$Clusters <- factor(df.ordered$Clusters, levels = colnames(nes.df))
...
combined.plot <- combined.plot + plot_layout(widths = c(0.2, 0.8)) + plot_annotation(
  title = "Enrichment of Hallmark pathways across tumor clusters",
  subtitle = paste(CONTEXT, "Normalized")
)

ggsave(paste(OUTPUT_DIR, "enrichment_bubble_plot_fgsea_nes_within_tumor_clusters.pdf", sep = "/"),
...
df.ordered$cluster_pathway <- paste(df.ordered$Clusters, 
...
ggsave(paste(OUTPUT_DIR, "enrichment_bubble_plot_fgsea_nes_within_tumor_clusters_logfc_overlay.pdf", sep = "/"),
       combined.plot, height = 6, 
       width = 6, device = pdf)
