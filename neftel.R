...
dat$tumor <- ifelse(grepl("-like$", dat$celltype), "tumor", "normal")
...
dat@meta.data$sample <- orig_ident_to_sample[dat@meta.data$orig.ident]


#subset data
tumor <- subset(dat, tumor == "tumor")

# add metadata based on Neftel et al., 2019
reference_directory <- "./ref/neftel_reference_sheets"
resultsdirectory <- "./neftel"

#Calculate a MESlike, AC, OPC and NPC-like score for each cell in the dataset based on the expression of genes provided by Neftel
...
##For MESlike and NPClike features, we extract the unique signatures from both MESlike/NPClike1 and 2. We then remove any NA values. 
...
signatures_list <- list(
  ...
  "AClike" = as.character(signatures$AClike),
  "OPClike" = as.character(signatures$OPClike),
  "NPClike" = as.character(NPClike_features),
  "MESlike1" = as.character(signatures$MESlike1),
  "MESlike2" = as.character(signatures$MESlike2),
  "NPClike1" = as.character(signatures$NPClike1),
  "NPClike2" = as.character(signatures$NPClike2),
  "G1.S" = as.character(signatures$G1.S),
  "G2.M" = as.character(signatures$G2.M)
)
...
## Calculate max scores for OPC-NPC vs AC-MESlike
...
tumor@meta.data$MaxScore_AC.MESlike <- apply(tumor@meta.data[, c("AClike.Score", "MESlike.Score")], 1, function(x) x[which.max(x)])
## Calculate D1. 
## RATIONALE: If for a CELL, MaxScore_OPC.NPC is the value of the OPC score and MaxScore_AC.MESlike is the value of the AC score, and IF OPC score > AC score then D1 (MaxScore_OPC.NPC - MaxScore_AC.MESlike) would be > 0. The CELL would be more OPClike than AClike. IF OPC score < AC score then D1 (MaxScore_OPC.NPC - MaxScore_AC.MESlike) would be < 0. The CELL would be more AClike than OPClike.
tumor@meta.data$D1 <- with(tumor@meta.data, MaxScore_OPC.NPC - MaxScore_AC.MESlike)
## Classify D1 > 0 as OPC-NPC cells, and D1 < 0 as AC-MESlike cells
tumor@meta.data$combGroup1 <- c("EMPTY")
tumor@meta.data$combGroup1[tumor@meta.data$D1 > 0] <- "OPC_NPC"
tumor@meta.data$combGroup1[tumor@meta.data$D1 < 0] <- "AC_MESlike"

## Calculate max scores for AC-OPC vs MESlike-NPC
...
tumor@meta.data$MaxScore_MESlike.NPC <- apply(tumor@meta.data[, c("MESlike.Score", "NPClike.Score")], 1, function(x) x[which.max(x)])

## Calculate D2
## RATIONALE: If for a CELL, MaxScore_AC.OPC is the value of the OPC score and MaxScore_MESlike.NPC is the value of the MESlike score, and IF OPC score > MESlike score then D2 (MaxScore_AC.OPC - MaxScore_MESlike.NPC) would be > 0. The CELL would be more OPClike than MESlike. IF OPC score < AC score then D2 (MaxScore_AC.OPC - MaxScore_MESlike.NPC) would be < 0. The CELL would be more MESlike than OPClike.
tumor@meta.data$D2 <- with(tumor@meta.data, MaxScore_AC.OPC - MaxScore_MESlike.NPC)
## Classify D2 > 0 as AC-OPC cells, and D1 < 0 as MESlike-NPC cells
tumor@meta.data$combGroup2 <- c("EMPTY")
tumor@meta.data$combGroup2[tumor@meta.data$D2 > 0] <- "AC_OPC"
tumor@meta.data$combGroup2[tumor@meta.data$D2 < 0] <- "MESlike_NPC"

##We will now calculate a relative metamodule x and y score. 
##We want NPC and OPC-like cells to have a positive y-value and AC and MESlike-like cells to have a negative y-value.
##For cells that are classified as 'OPC_NPC' we log2 transform the difference between OPClike and NPClike scores.
##For cells that are classified as 'OPC_NPC' we (-)log2 transform the difference between OPClike and NPClike scores. A positive value will indicate OPC/NPC and a negative value AC/MESlike.
...
tumor@meta.data$rel.metamodule.y.score[tumor@meta.data$combGroup1 == "AC_MESlike"] <- with(tumor@meta.data[tumor@meta.data$combGroup1 == "AC_MESlike",], -log2((abs(AClike.Score - MESlike.Score)) + 1)) ## negative number for AC-MESlike

##We want NPC and MESlike-like cells to have a positive x-value and AC and OPC-like cells to have a negative y-value.
##For cells that are classified as 'AC_OPC' we -log2 transform the difference between OPClike and AClike scores.
##For cells that are classified as 'MESlike_NPC' we log2 transform the difference between NPClike and MESlikeClike scores. A positive value will indicate MESlike/NPC and a negative value AC/OPC.
...
tumor@meta.data$rel.metamodule.x.score[tumor@meta.data$combGroup2 == "MESlike_NPC"] <- with(tumor@meta.data[tumor@meta.data$combGroup2 == "MESlike_NPC",], log2((abs(MESlike.Score - NPClike.Score)) + 1))
...
## Right-Bottom Quadrant: MESlike-like - Positive x-value and Negative y-value.
...
#saveRDS(tumor, file = paste0(resultsdirectory,"/tumor.rds"))
...
label_data <- percentage_data %>%
  mutate(type = str_replace(Quadrant, "like", ""),  # Remove 'like' suffix to get the clean type
         x = case_when(
           ...
           type == "MESlike" ~ 1    # MESlike -> Lower right
         ),
         y = case_when(
           type == "OPC" ~ 1,   # OPC -> Upper left
           type == "NPC" ~ 1,   # NPC -> Upper right
...
scatterPlot <- ggplot(tumor@meta.data, aes(x = rel.metamodule.x.score, y = rel.metamodule.y.score)) + ## transform to change the order of the plots in facet_wrap()
  ...
  theme_bw() +
  theme(legend.position = "top", axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 20), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20), panel.border = element_rect(fill=NA, colour = "black", size=1.5)) +
...
ggsave(scatterPlot, filename = paste0(resultsdirectory,"/plots/GBM_metamodules_bysample_scale0-2000.pdf"), height = 12, width = 12, dpi = 300, useDingbats = FALSE)
...
ggsave(scatterPlot, filename = paste0(resultsdirectory,"/plots/GBM_metamodules_combined_scale0-5000.pdf"), height = 6, width = 5, dpi = 300, useDingbats = FALSE)
...
scatterPlot_cycling_density <- ggplot(tumor@meta.data, aes(x = rel.metamodule.x.score, y = rel.metamodule.y.score)) +
  ...
  scale_colour_gradientn(
    ...
    limits = c(0, 500),  # Adjust limits as needed based on your data
    breaks = c(0, 500),  # Show density range in the legend
    labels = c("0", "500")) +
  guides(color = guide_colorbar(
...
# Save the plot to a file
ggsave(scatterPlot_cycling_density, filename = paste0(resultsdirectory, "/plots/GBM_metamodules_combined_cycling_scale0-500.pdf"), height = 5, width = 5, dpi = 300, useDingbats = FALSE)
...
# Save the plot to a file
ggsave(cell_cycle_scatter, filename = paste0(resultsdirectory, "/plots/GBM_cell_cycle_G1S_G2M.pdf"), height = 5, width = 5, dpi = 300, useDingbats = FALSE)

# 3) plot cycling based on G2.M

# Calculate cycling percentage per quadrant
...
# 4) plot by sample, color by subclones (no NA)

# Load the subclone data
subclone_data <- readRDS("SCEVAN_by_sample/combined_results.rds")
...
scatterPlot_subclone <- ggplot(tumor@meta.data, aes(x = rel.metamodule.x.score, y = rel.metamodule.y.score, color = factor(subclone))) +
  ...
  guides(color = guide_legend(title = "Subclone", title.position = "top", title.hjust = 0.5)) +
  facet_wrap(~ sample) +
  theme_bw() +
  theme(legend.position = "top", axis.text.x = element_text(size = 20), axis.text.y = element_text(size = 20), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20), panel.border = element_rect(fill = NA, colour = "black", size = 1.5)) +
  xlab(expression(atop("Relative meta-module score", paste("[log2(|SC1-SC2|+1)]")))) +
...
# Save the plot
ggsave(scatterPlot_subclone, filename = paste0(resultsdirectory, "/plots/GBM_metamodules_bysample_subclones.pdf"), height = 12, width = 12, dpi = 300, useDingbats = FALSE)
...
# Save the plot
ggsave(scatterPlot_subclone, filename = paste0(resultsdirectory, "/plots/GBM_metamodules_bysample_subclones_noNA.pdf"), height = 12, width = 12, dpi = 300, useDingbats = FALSE)
...
# Initialize a data frame for storing results
  ...
  for (feature in features) {
    ...
    generate_violin_plot(dat, feature, comp, paste0(feature, "_sample_type.png"), group = "sample_type", custom_colors = c("#68F6B4", "#F58D9F"), p_value = sample_type_result$p_value)
    ...
    generate_violin_plot(dat, feature, comp, paste0(feature, "_split_sample_type.png"), group = "MES", split = "sample_type", custom_colors = c("#68F6B4", "#F58D9F"))
    ...
    generate_violin_plot(dat, feature, comp, paste0(feature, "_split_MES.png"), group = "sample_type", split = "MES", custom_colors = c("#387ee0", "gray"))
...
mek_sens <- c("BRAF", "SHOC2", "SOS1", "SKP2")
